...
<%
	...
	LinkedHashMap<String, SortedMap<String, Integer>> result = UserUsePlatformDao.statics(userID, actionType, false, year);
...
<title>SCOPUS 정보 검색 플랫폼 - 월별 이용현황</title>
...
function viewYear(){
	...
	form.action="./usePlatformYear.jsp";	
	form.submit();
}
...
<div id="wrap">
    ...
    <div id="container">
		...
		<div id="content">
	    	...
	    	<div id="search">
	    	<%
				String menuTerm = "사용자 이용 현황 (월별 이용 현황)";
			%>
                    	...
                    	<table class="Table1_bt" border="0" cellpadding="0" cellspacing="0" align="center">
                            <tr>
                                <td class="txt" style="text-align: left">
                                	<input type="submit" value="연도별 현황 보기" class="bt5"  onclick="javascript:viewYear();"/>
                                </td>
                                <td> 	
       	  		...
       	  		<tr>
                    	...
                    	<table class="Table2_bt" border="0" cellpadding="0" cellspacing="0">
                            <tr>
								...
								<%
									for(int _year = (Calendar.getInstance().get(Calendar.YEAR)) ; _year >= 2011 ; _year--){
			...
			<table width="100%" border="0" cellpadding="0" cellspacing="0" class="Table5">
                <tr>
                    <td>
                        <table id="dataTable" cellspacing="0" cellpadding="0" border="0" width="100%" class="Table5 line">
                            <caption>사용자 이용 현황 테이블</caption>
                            <thead>
                            <!-- 
                                <tr bgcolor="#c0defa">
                                    <td rowspan="2" style="padding:2px 2px 2px 2px;">User ID</td>
                                    <td colspan="13" style="padding:2px 2px 2px 2px;"><%=year %>년</td>
                                </tr>
                                 -->
                                <tr bgcolor="#c0defa">
                                ...
                                <%
                                	for(int month=1 ; month < 13; month++){
                                		out.println("<th style=\"padding:2px 2px 2px 2px;\">"+month+"월</th>");
                                	}
                            ...
                            <%
                            	...
                            	for(String _userID : result.keySet()){
                            		...
                            		String name = " (" + ub.getNames() +")";
                            		String email = ub.getEmail();
                            		String department = ub.getDepartment();
                            		String title = ub.getNames() +"\n" + email +"\n" + department;
                            		out.println("<th class='tipTipClass' title=\""+title+"\">"+_userID + name+"</th>");
                            		
                            		SortedMap<String, Integer> data = result.get(_userID);
                            		for(String ym : data.keySet()){
	                            		out.println("<td>"+data.get(ym)+"</td>");
                            		}
                            		out.println("</tr>");
                            		cnt++;
                            	}
