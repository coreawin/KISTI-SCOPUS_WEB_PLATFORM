...
<script type="text/javascript">
	function exportExcel(){
		var checkedSeq = "<%=param%>";
		exportCluster(checkedSeq);
	}
	
	function exportCluster(checkedSeq){
		if(jQuery.trim(checkedSeq) == ''){
			alert("오류가 발생했습니다.");
			return;
		}
		if(confirm("클러스터 결과를 Export 하시겠습니까?")){
...
<%
	if(userBean!=null){
%>
<table cellpadding="5" cellspacing="5" border="0"  width="100%" id="exportHeaderTable">
	<tr>
		<td height="30" align="right" style="vertical-align: middle; " ><h3>선택된 모든 분석항목 다운로드 (excel) </h3> </td>
		<td align="center" style="vertical-align: middle; " width="25"><a href="javascript:exportExcel();"><img src="../images/download2.gif" border="0" ></a></td>
	</tr>
</table>
<%
	}
	...
	<tr>
		<td>
			<table id="mytable" cellpadding="0" cellspacing="0" border="0" align="left" width="100%">
				<tr>
					<td colspan="3"></td>
					<th class="specalt" width="100">download excel
					<td width="200"><a href="javascript:exportCluster('<%=checkedSeq%>');"><img src="../images/download.gif" border="0"></a></td>
				</tr>
				<tr>
...
<%!
	...
	private String printTopDocumentTable(Map<String, Integer> citationDocInfoMap, Map<String, String> docTitleInfoMap, int topIndex, UserBean userBean){
		...
		for (String eid : eidSet) {
			...
			topDocumentSB.append("</td>");
			if(userBean==null){
				topDocumentSB.append("<td><a href=\"../noregist/viewScopus.jsp?eid="+eid.trim()+"\" target=\"blank\">");
			}else{
				topDocumentSB.append("<td><a href=\"../documentSearch/viewDocument.jsp?eid="+eid.trim()+"\" target=\"blank\">");
			}
			topDocumentSB.append(eid.trim());
