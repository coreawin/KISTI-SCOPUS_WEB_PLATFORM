function create_data(graph_type, graph_title, array_labels, array_values,
		obj_setup, array_id_title, array_values_title) {
	...
	if (graph_type == 'bar_stack') {
		...
		var array_colors = getcolour_stages(html2rgb(obj_setup.x_axis_colour),
				html2rgb(obj_setup.y_axis_colour), array_values.length - 1);
	...
	if (graph_type == 'pie') {
		for (i = 0; i < array_values[0].length; i++) {
			if (array_values[0][i] > 0) {
				array_pie.push({
					"value" : array_values[0][i],
					"label" : array_labels[i] + ":" + array_values[0][i]
				});
			}
		}
		obj_elements[0] = {
			...
			"colours" : [ '#FF0F00', '#0D52D1', '#8A0CCF', '#04D215', '#0D8ECF' ,'#000000', '#FF9E01', '#CD0D74', '#754DEB', '#999999'],
			"gradient-fill" : true,
			"start-angle" : 35,
		...
		if (graph_type == 'bar_stack') {
			...
			for (i = 0; i < array_values_title.length; i++) {
				array_keys.push({
					"colour" : array_colors[i],
					"text" : array_values_title[i],
					"font-size" : 13
				});
			}
...
function flash_chart_data(passed_string) {
	...
	for (i = 1; i <= th_count; i++) {
		...
		if (do_push) {
			table_elm
					.find("tbody tr td:nth-child(" + i + ")")
					.each(
							function(x) {
								if (jQuery.inArray((x + 1).toString(),
										array_skip_row) == -1) {
									if (b_array_id) {
										array_id.push(jQuery.trim($(this)
												.text()));
									}

									if (b_array_values) {
										values_rows_array_counter += 1;
										// values must to numbers so strip any
										// text
										if (b_stacked) {
											// array_values[values_rows_array_counter].push(parseInt($(this).text().replace(re,'')));
											array_values[values_rows_array_counter]
													.push(parseFloat($(this)
															.text().replace(re,
																	'')));
	...
	the_data = create_data(chart_type, chart_name, array_id, array_values,
			obj_setup, array_id_title, array_values_title);
...
$(function() {
	...
	// default setup
	var flash_chart_h = 225;
	var flash_chart_w = 450;
					...
					function(i) {
						...
						$($(this))
								.after(
										'<div id="'
												+ current_name
												+ '">You do not have flash installed - please go to: <a href="http://get.adobe.com/flashplayer/">http://get.adobe.com/flashplayer/</a> to install it</div>');
						...
						if (current_el.css("background-image") != 'none') {
							flash_chart_w = 450;// parseInt(current_el.css("width"));
							flash_chart_h = 225;
							flash_chart_bg = rgb2html(current_el
									.css("background-color"));
							flash_chart_y_grid = rgb2html(current_el
									.css("border-top-color"));
							flash_chart_y_axis = rgb2html(current_el
									.css("border-left-color"));
							flash_chart_x_axis = rgb2html(current_el
									.css("border-right-color"));
							flash_chart_x_grid = rgb2html(current_el
									.css("border-bottom-color"));
						}
						current_el.removeClass("flash_chart_setup");

						var pass_string = $(this).attr("id") + ','
								+ flash_chart_bg + ',' + flash_chart_y_grid
								+ ',' + flash_chart_y_axis + ','
								+ flash_chart_x_axis + ',' + flash_chart_x_grid
								+ ',' + flash_chart_h + ',' + flash_chart_w
								+ ',' + chart_type;
						// add the flash to the page pointing at the table for
						// its data
						// AllowScriptAccess
						var flashvars = {
							"get-data" : "flash_chart_data",
							"id" : pass_string
						};
						var params = false;
						var attributes = {
						...
						swfobject.embedSWF(flash_chart_path, current_name,
								flash_chart_w, flash_chart_h, "9.0.0",
...
function rgb2html(rgb_string) {
	...
	hex = rgb_string;
	if (hex.indexOf('#') == -1) {
		// replace the rgb test etc with nothing, to leave the commas and
		// numbers
		...
		/*
		 * //make sure the hex string is full length if(hex.length<6){ var
		 * new_hex=hex.substring(1,hex.length); add_char=new_hex.charAt(1);
...
function show_hide_chart(flash_chart, html_table) {
	...
	$(html_table)
			.before(
					'<div class="show_hide_container"><span class="flash_chart_title">'
							+ $(html_table).attr("summary")
							+ '</span><a href="##" id="show_hide_flash_'
							+ current_name
							+ '" class="icon_link flash_chart_link"><img src=\"../images/nn_graph_icon01.gif\" align=\"middle\" alt=\"Show chart\" title=\"Show chart\" border=\"0\"/></a> <a href="##" id="show_hide_table_'
							+ current_name
							+ '" class="icon_link html_table_link"><img src=\"../images/nn_graph_icon02.gif\" align=\"middle\" alt=\"Show table\" title=\"Show table\" border=\"0\"/></a> ' 
							+ '<a href=\"javascript:excelExport(\''+current_name+'\');\" ><img src=\"../images/nn_graph_icon03.gif\" align=\"middle\" alt=\"download table for excel\" title=\"download table for excel\" border=\"0\"/></a>' 
							+ '</div>');
						...
						+ [
								Math
										.max(
												Math
														.min(
																parseInt((new_pos * (parseInt(end_array[0]) - parseInt(start_array[0])))
																		+ parseInt(start_array[0])),
																255), 0),
								Math
										.max(
												Math
														.min(
																parseInt((new_pos * (parseInt(end_array[1]) - parseInt(start_array[1])))
																		+ parseInt(start_array[1])),
																255), 0),
								Math
										.max(
												Math
														.min(
																parseInt((new_pos * (parseInt(end_array[2]) - parseInt(start_array[2])))
																		+ parseInt(start_array[2])),
																255), 0) ]
								.join(",") + ")"));
	}
	return array_colour;
