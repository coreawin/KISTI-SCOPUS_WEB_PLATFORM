...
<%
	...
	if(insertSearchRule){
		session.removeAttribute("guest_selectDocSet");
	}
	String se = "";
	String ft = "";
	...
	if(se == null || se.length() == 0){
	    se = "{"+fields+":"+searchTerm+"}";
	}

	String fl = "eid,title,country,authorname,authorid,pubyear,affiliation,sourceid,sourcetitle,citcount,refcount,sourcetype,asjc,_score_,refeid,citeid";
	...
	//LinkedHashSet<String> selectedDocEIDSet = (LinkedHashSet<String>)session.getAttribute("guest_selectDocSet");
	...
	searchRule = "se="+se+"&ft="+ft;
	 ...
	 String ht = ""+URLEncoder.encode("<font color\\='red'><b>:</b></font>");
	 String ud = "";
...
<body>
	...
	<div id="wrap">
		...
		<div id="container">
	    	...
	    	<div id="search">
	    		...
	    		<%@include file="../common/topAreaNoLink.jsp" --%>
			</div>
		 -->
					...
					<table border="0" cellpadding="0" cellspacing="0" style="margin-left: 15px">
						<tr>
							<td colspan='3' height="10"
								...
								<%--=(searchTerm != null && searchTerm.trim().length() > 0) ? "Query : " + searchRule.replaceAll("\\\\:",":").replaceAll("\\\\,",",").replaceAll("\\\\&","&").replaceAll("\\\\=","=") : ""--%>
								</span> -->
							</td>
						</tr>
					</table>
															...
															<tr bgcolor="#ecf8ff">
																<td height="40"
																	...
																	<span style="text-shadow: 1px 1px 2px #999;">
																		<img src="../images/001_40.png" align="bottom" width="16" height="16"/>
																			Document Search Results : <%=NumberFormatUtil.getDecimalFormat(iTotalTotal)%>
																	</span>
																	<!-- 
											...
											<table cellspacing="0" cellpadding="0" width="100%" id="searchResultTable" class="SearchResultTable">
												...
												<tbody>
													...
													<tr <%=bgColor%>>
														...
														<td valign="top"
															style="padding: 5px 5px; text-align: left;"><a
															href="./viewScopus.jsp?eid=<%=jsonrecord.getString("eid")%>">
 ...
 %> <!-- 통계 항목 End --> <!-- 결과내 재검색을 위한 하단 버튼 표시 Start -->
		...
		<%
					...
					out.println("<script>");
					out.println("alert('검색된 결과가 존재하지 않습니다.'); ");
					out.println("</script>");
				}
			}
		%>
