...
function viewSrcLink(doi, type){
	var name = "HCPSEARCH_<%=currentTime%>";
	var spec = "width=680,height=600,status=no,resizable=no,scrollbars=yes";
	var url = "";
	var wopen = window.open(url,name,spec, true);
	var form = document.getElementById("linkParameter");
	form.id.value = "doi:"+doi;
	form.Target.value = type;
	form.action = "<%=apiURL%>";
	...
	function getFullTextInfo(doi){
		$.ajax({
			...
			success: function(msg){
				...
				var doiFormObject = document.getElementById("fullText");
				if(result!=""){
...
<div id="wrap">
    ...
    <div id="container">
		...
		<div id="content2">
            ...
            <table class='sdBase' width="100%" border="0" summary="검색 결과 테이블" cellpadding="0" cellspacing="0" width="960" style="margin-left:20px; margin-right: -20px">
            	...
            	<tr>
            		<td>
            			<div class="scopus_subtitle"">Abstract</div>
						<div class="" style="margin-right: 50px;"><%=sdBean.getAbs() %></div>
						<hr></hr>
            		</td>
            	</tr>
            	<tr>
            	...
            	<tr>
            		<td>
            			<span class="scopus_description_title"">Times cited</span> : <a href="javascript:searchRefCit('CITL');"><%=sdBean.getCitCount()%></a>
            			<hr></hr>
            		</td>
            	</tr>
            	<tr>
            		<td>
            			<p>
            			<div class="scopus_subtitle"">References (<a href="javascript:searchRefCit('REFL');"><%=refList.size() %></a>)</div>
            			</p>
            			<p>
    ...
    <!--contents_area-->
     ...
     <form id="linkParameter" method="get">
    	<input type="hidden" name="keyValue" value="<%=apiID%>"/>
    	<input type="hidden" name="id"/>
    	<input type="hidden" name="returnType" value="html"/>
